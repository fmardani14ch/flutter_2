...
class USER_SERVICE {
  ...
  static ValueNotifier<List<USER_MODEL>> user_list = ValueNotifier([]);
  //******************************************** گرفتن اطلاعات تمام دیتا بیس */
  static Future<List<USER_MODEL>> get_user_list() async {
    //************************json متغیری برای دریافت اطلاعات به صورت جیسان */
    var response = await httpClient.get("tlQLOf/user");
    ...
    if (response.statusCode == 200 || response.statusCode == 201) {
      ...
      for (var element in response.data) {
        ...
        user_list.value.add(USER_MODEL.fromJson(element));
      }
      //************ بازگردانی یا خواندن مقادیر درون لیست مربوط به خط اول */
      return user_list.value;
    }
...
//*** یک متغیر که بهتر است هم نام کلاس ولی با نام کوچکتر باشد ارسال میشود */
  static Future<USER_MODEL> add_user(USER_MODEL user_model) async {
    //سپس با یک متغیر، یک درخواست  که میخواهیم اطاعات رااز نوع متد پست و
    //***************************  تبدیل دارت به جیسان به سرور ارسال میکنم */
    var response =
        await httpClient.post("tlQLOf/user", data: user_model.toJson());
...
//##############################################################################
  ...
  static Future<USER_MODEL> delete_user_product(int id) async {
